...
areaShape (Rectangle _ x y) = x * y
areaShape (Circle _ r) = 3.14 * r * r
areaShape (Triangle a b c) = (dist(a,b) * dist(a,c)) / 2 --Para triangulo retagulo cuja hipotenusa seja cb;
...
qtdElementos :: IntTree -> Int
qtdElementos ILeaf = 0
qtdElementos (INode _ l r) = 1 + (qtdElementos l) + (qtdElementos r)

qtdFolhas :: IntTree -> Int
qtdFolhas ILeaf = 1
qtdFolhas (INode _ l r) = (qtdFolhas l) + (qtdFolhas r )
...
converteListToTree :: IntList -> IntTree
converteListToTree INil = ILeaf
converteListToTree (ICons x xs) = inserirArvore x (converteListToTree xs)
  where
    inserirArvore :: Int -> IntTree -> IntTree
    inserirArvore x ILeaf = INode x ILeaf ILeaf
    inserirArvore x (INode y l r)
      = case compare x y of
          LT -> INode y (inserirArvore x l) r
          GT -> INode y l (inserirArvore x r)
          EQ -> INode y l r
...
offers :: Client -> Bool
offers (Customer _ _ v) = v

name :: Client -> Name
name (Customer nome _ _) = nome

ofertaCliente :: [Client] -> [Name]
ofertaCliente [] = []
ofertaCliente (h : t)
  |offers h == True = name h : ofertaCliente t
  |otherwise = ofertaCliente t
