...
public class PubSubConsumer<S extends Socket> extends GenericConsumer<S>{
	...
	// Método executado quando o servidor gera um socket.
	// Lê uma mensagem, executa o comando e envia a resposta obtida por ele no primário ou no backup (se mensagem do tipo sync)
    @Override
    protected void doSomething(S str) {
		...
		try{
			...
			// Insere o registro no backup (se for uma mensagem do tipo sub) ou então no primário e acorda o respectivo broker
