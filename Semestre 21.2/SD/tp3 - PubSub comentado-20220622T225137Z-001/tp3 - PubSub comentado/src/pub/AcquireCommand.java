...
public class AcquireCommand implements PubSubCommand{
	...
	public Message execute(Message m, SortedSet<Message> log, List<String> subscribers, Address backup) {
		...
		// Se tiver um backup ativo (??)
		
		if (! backup.empty() ){
			// Se tiver backup, o primário pega o conteúdo da mensagem e envia pro backup com a intenção de sincronizar
			// Se algo der errado, apaga o backup
			try {
