...
restricao_03{t in T : t > 1}: ((sum{j in K, s in S[t]} Xjs[t,j,s]) + (sum{j in K,k in K, s in S[t]} stjk[j,k] * Zjks[t, j,k,s])) <= capt[t];
...
resticao_07{j in K, t in T, s in S[t] : t > 1}: Yjs[t,j,s] = sum{k in K} Zjks[t,j,k,s];

restricao_08{k in K, t in T, s in S[t] : t > 1}: Yjs[t,k,s] = sum{j in K} Zjks[t,j,k,s];
