...
minList :: [Int] -> Int
minsList [] = 0
minList (h:t) = minList' t h
     where
          minList' :: [Int] -> Int -> Int
          minList' [] min = min 
          minList' (x : xs) min 
               | min <= x = minList' xs min
               | otherwise = minList' xs x
...
indexOf x (h:t)
     | x == h = 0
     | otherwise = indexOf' x 1 t
     where
          indexOf' :: Int -> Int -> [Int] -> Int
          indexOf' x cont (h:t)
               | x == h = cont
               | t == [] = -1
               | otherwise = indexOf' x (cont + 1) t
\end{code}
...
takeList _ [] = []
takeList 0 _ = []
takeList n (h:xs) = h : takeList (n - 1) xs
...
dropList _ [] = []
dropList 0 xs = xs
dropList n (h:xs) = dropList (n - 1) xs
