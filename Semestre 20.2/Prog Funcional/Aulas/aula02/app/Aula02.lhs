...
acharFatores :: Int -> [Int]
acharFatores n = [x | x <- [1 .. n-1], (mod n x == 0)]

ehPrimo :: Int -> Bool
ehPrimo n = if (sum (acharFatores n) == n) then True else False
...
obterPrimos :: [Int] -> [Int]
obterPrimos intervalo = [x | x <- intervalo, length (factors x) == 2]
...
max3 x y z = if x > y then
              if y > z then x else if x > z then x else z
...
--moduloVetor :: Vetor -> Double
--moduloVetor (x,y) = 2

calcularNormalizada :: Vetor -> (Double, Double)
calcularNormalizada (x,y) = ( (x/moduloVetor), (y / moduloVetor))
    where
      moduloVetor = sqrt (x ^2 + y ^ 2)
...
verifica2 :: [Int] -> Bool
verifica2 lista = if length lista <= 2 then True else False
