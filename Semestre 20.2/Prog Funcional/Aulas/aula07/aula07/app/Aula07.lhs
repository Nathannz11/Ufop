...
evenList :: [Int] -> [Bool]
evenList [] = []
evenList (x : xs) = even x : evenList xs
...
selectionF :: (Int,Int) -> [Int] -> [Int]
selectionF _ [] = []
selectionF (a,b) xs = filter pertence xs
  where
    pertence :: Int -> Bool
    pertence n 
      | n >= a && n <= b = True
...
selectionR :: (Int, Int) ->[Int] -> [Int]
selectionR _ [] = []
selectionR (a, b) xs = selectionR' xs
  where  
    pertence :: Int -> Bool
    pertence n 
      | n >= a && n <= b = True
    ...
    selectionR' [] = []
    selectionR' (x : xs)
      | pertence x = x : selectionR' xs
      | otherwise = selectionR' xs
...
quadOdd :: [Int] -> Int
quadOdd [] = 0
quadOdd xs =  sum (mapL (\ x -> x ^ 2)  (filterL odd xs))
\end{code}
...
allF :: (a -> Bool) -> [a] -> Bool
allF f  = foldr step True 
  where
    step x acc = if f x then True && acc else False
\end{code}
